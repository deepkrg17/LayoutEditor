...
import com.itsvks.editor.databinding.LayoutComponentTreeBinding;
import com.itsvks.editor.managers.IdManager;
import java.util.HashMap;
import java.util.Map;

public class ComponentTree extends LinearLayout implements View.OnClickListener {
   ...
   */
  public ComponentTree(Context context, AttributeSet attrs) {
    super(context, attrs);
    ...
    paint.setStrokeWidth(SizeUtils.px2dp(1));

    pointRadius = SizeUtils.px2dp(3);
  ...
  private void peek(View view, int depth) {
    int nextDepth = depth;
    LayoutComponentTreeBinding binding = LayoutComponentTreeBinding.inflate(inflater, null, false);
    ...
    if (view.getId() == -1 || IdManager.getIdMap().get(view) == null) {
      ...
      viewName.setTranslationY(SizeUtils.px2dp(-7));
      viewId.setTranslationY(SizeUtils.px2dp(-3));
    ...
    LinearLayout.LayoutParams params =
        (LinearLayout.LayoutParams) binding.getRoot().getLayoutParams();
    params.leftMargin = depth * SizeUtils.px2dp(15);
