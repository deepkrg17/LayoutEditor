...
public class DrawableManagerActivity extends BaseActivity {
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    ic_check =
        AppCompatResources.getDrawable(DrawableManagerActivity.this, R.drawable.ic_progress_check);
  ...
  private void addDrawable(final String path) {
    ...
    final TextInputLayout inputLayout =
        (TextInputLayout) getLayoutInflater().inflate(R.layout.textinputlayout, null, false);
  ...
  private void checkNameErrors(String name, TextInputLayout inputLayout, AlertDialog dialog) {
    if (!name.isEmpty()) {
      if (Character.isDigit(name.charAt(0))) {
        ...
        inputLayout.setError(getString(R.string.msg_first_letter_not_number));
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(false);
        return;
      }
      if (name.contains(" ")) {
        inputLayout.setErrorEnabled(true);
        inputLayout.setError(getString(R.string.msg_space_not_allowed));
        dialog.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(false);
        return;
      }
      ...
      inputLayout.setErrorEnabled(true);
      inputLayout.setError(getString(R.string.msg_cannnot_empty));
      dialog.getButton(AlertDialog.BUTTON_POSITIVE).setEnabled(false);
      return;
    }
